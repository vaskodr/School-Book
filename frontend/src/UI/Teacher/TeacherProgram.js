...
class TeacherProgram extends Component {
    constructor(props) {
        super(props);
        this.state = {
            program: null,
    ...
    fetchProgramData = async () => {
        const { schoolId, teacherId, authData } = this.props;
        console.log('Fetching program data for teacherId:', teacherId); // Debugging log

        try {
            console.log('School id: ', schoolId);
            console.log('Teacher id: ', teacherId);
            const response = await fetch(`http://localhost:8080/api/school/${schoolId}/program/teacher/${teacherId}/weekly-program`, {
                headers: {
                                ...
                                {program.weeklyProgram[day.toUpperCase()]?.map((session, index) => (
                                    this.isTimeInRange(session, timeSlot) && (
                                        <div key={index} className="mb-2">
                                            ...
                                            <p className="mb-1"><strong>Class:</strong> {session.class}</p>
        ...
        return (
            <div className="container mt-5">
                <button className="btn btn-primary mb-3" onClick={this.fetchProgramData}>
                    {this.state.showProgram ? 'Reload Teacher Program' : 'Load Teacher Program'}
                </button>
